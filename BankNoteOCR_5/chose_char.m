...
% 做灰度拉伸并计算平均灰度 average
[M,N] = size(in_image);
t = double(min(min(in_image)));
T = double(max(max(in_image)));
sum = 0;
for i =1:M
    for j = 1:N
        x = double(in_image(i,j));
        in_image(i,j) = round((x-t)/(T-t)*255);  %灰度拉伸(x-t)/(T-t)=x'/255  - >  x'=255*(x-t)/(T-t)
